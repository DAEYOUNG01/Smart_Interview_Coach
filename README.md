...
> **Agent가 일관성 평가를 수행하기 위해 5가지 핵심 평가 지표와 5점 척도를 정의하여 시스템 프롬프트에 적용**
...
<table>
  ...
  <tbody>
    <tr>
      <td>💡 <b>피드백 구체성</b></td>
      <td>단순 "좋음/나쁨"이 아닌, "수치적 성과가 부족", "협업 관점의 설명이 필요" 등 구체적 약점 지적</td>
    </tr>
    <tr>
      <td>💡 <b>질문 부드러움</b></td>
      <td>이전 맥락을 반영 "~라고 하셨는데, 구체적으로 어떤 기술을 사용했나요?"와 같은 연결형 질문 생성</td>
    </tr>
    <tr>
      <td>💡 <b>리포트 품질</b></td>
...
> **본 AI 면접관 Agent는 채용 담당자에게는 운영 효율을, 구직자에게는 언제든 접속 가능한 실전 면접 환경을 제공**
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        • 정량화된 평가 기준(Rubric) 적용으로 평가 일관성 확보<br>
    ...
    <tr>
      ...
      <td>
        ...
        • 교사 및 상담사의 지도 공백 해소와 운영 부담 경감을 위한 역할 보조<br>
        • 취업 지원 센터의 상담 효율성 증대
      </td>
    </tr>
    <tr>
      ...
      <td>
        ...
        • 객관적인 피드백 리포트를 통해 자신의 강점과 약점 파악<br>
...
I. Agent는 이력서 파일(PDF/DOCX)과 텍스트 채팅에만 의존

실제 면접에서 중요한 비언어적 요소(태도, 목소리, 표정 등)를 전혀 평가 불가능 -> 음성/영상 입력 기능 추가

II. 단일 LLM에 의존하는 평가의 주관성

평가 자체가 프롬프트에 민감하거나 편향 가능 -> Reflection 노드를 추가하여 점수 편향을 최소화하는 로직을 고려

III. RAG 부재로 인한 팩트 체크 및 할루시네이션 위험

순수 LLM 생성 방식 -> 전문성 높은 질문 pool을 Vector DB로 구축 + 이력서 키워드, 컨텍스트 기반 질문을 검색하여 LLM에 주입
